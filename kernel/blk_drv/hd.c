...
// Èç¹ûÃ»ÓĞÔÚ config.h ÖĞ¶¨ÒåÓ²ÅÌ²ÎÊı£¬¾Í´Ó 0x90080 ´¦¶ÁÈë¡£	
	...
	*/
	...
	// ¶ÁÈ¡Ã¿Ò»¸öÓ²ÅÌÉÏµÚ 1 ¿éÊı¾İ(µÚ 1 ¸öÉÈÇøÓĞÓÃ)£¬»ñÈ¡ÆäÖĞµÄ·ÖÇø±íĞÅÏ¢¡£
	// Ê×ÏÈÀûÓÃº¯Êı bread()¶ÁÓ²ÅÌµÚ 1 ¿éÊı¾İ(fs/buffer.c,267)£¬
	// ²ÎÊıÖĞµÄ 0x300 ÊÇÓ²ÅÌµÄÖ÷Éè±¸ºÅ(²Î¼ûÁĞ±íºóµÄËµÃ÷)¡£
	// È»ºó¸ù¾İÓ²ÅÌÍ· 1 ¸öÉÈÇøÎ»ÖÃ 0x1fe ´¦µÄÁ½¸ö×Ö½ÚÊÇ·ñÎª'55AA'£¬
	// À´ÅĞ¶Ï¸ÃÉÈÇøÖĞÎ»ÓÚ 0x1BE ¿ªÊ¼µÄ·ÖÇø±íÊÇ·ñÓĞĞ§¡£
	// ×îºó½«·ÖÇø±íĞÅÏ¢·ÅÈëÓ²ÅÌ·ÖÇøÊı¾İ½á¹¹ hd ÖĞ¡£
	for (drive=0 ; drive<NR_HD ; drive++) {
		...
		for (i=1;i<5;i++,p++) {//Ã¿¿éÓ²ÅÌ¶Á5¸ö·ÖÇø±í
...
// ÏòÓ²ÅÌ¿ØÖÆÆ÷·¢ËÍÃüÁî¿é(²Î¼ûÁĞ±íºóµÄËµÃ÷)¡£
...
// Ö´ĞĞÓ²ÅÌ¶ÁĞ´ÇëÇó²Ù×÷¡£
// ÈôÇëÇóÏîÊÇ¿éÉè±¸µÄµÚ 1 ¸ö£¬Ôò¿éÉè±¸µ±Ç°ÇëÇóÏîÖ¸Õë£¨²Î¼û ll_rw_blk.c£¬28 ĞĞ£©»áÖ±½ÓÖ¸Ïò¸Ã
// ÇëÇóÏî£¬²¢»áÁ¢¿Ìµ÷ÓÃ±¾º¯ÊıÖ´ĞĞ¶ÁĞ´²Ù×÷¡£·ñÔòÔÚÒ»¸ö¶ÁĞ´²Ù×÷Íê³É¶øÒı·¢µÄÓ²ÅÌÖĞ¶Ï¹ı³ÌÖĞ£¬
// Èô»¹ÓĞÇëÇóÏîĞèÒª´¦Àí£¬ÔòÒ²»áÔÚÖĞ¶Ï¹ı³ÌÖĞµ÷ÓÃ±¾º¯Êı¡£²Î¼û kernel/system_call.s£¬221 ĞĞ¡£
void do_hd_request(void)
{
	...
	// ¼ì²âÇëÇóÏîµÄºÏ·¨ĞÔ£¬ÈôÒÑÃ»ÓĞÇëÇóÏîÔòÍË³ö(²Î¼û blk.h,127)¡£
	INIT_REQUEST;
	// È¡Éè±¸ºÅÖĞµÄ×ÓÉè±¸ºÅ(¼ûÁĞ±íºó¶ÔÓ²ÅÌÉè±¸ºÅµÄËµÃ÷)¡£×ÓÉè±¸ºÅ¼´ÊÇÓ²ÅÌÉÏµÄ·ÖÇøºÅ¡£
	dev = MINOR(CURRENT->dev);
	block = CURRENT->sector;
	// Èç¹û×ÓÉè±¸ºÅ²»´æÔÚ»òÕßÆğÊ¼ÉÈÇø´óÓÚ¸Ã·ÖÇøÉÈÇøÊı-2£¬Ôò½áÊø¸ÃÇëÇó£¬
	// ²¢Ìø×ªµ½±êºÅ repeat ´¦£¨¶¨ÒåÔÚ INIT_REQUEST ¿ªÊ¼´¦£©¡£
	// ÒòÎªÒ»´ÎÒªÇó¶ÁĞ´ 2 ¸öÉÈÇø£¨512*2 ×Ö½Ú£©£¬
	// ËùÒÔÇëÇóµÄÉÈÇøºÅ²»ÄÜ´óÓÚ·ÖÇøÖĞ×îºóµ¹ÊıµÚ¶ş¸öÉÈÇøºÅ¡£
...
void hd_init(void)
{
	...
	// ÉèÖÃÓ²ÅÌÖĞ¶ÏÃÅÏòÁ¿ int 0x2E(46)¡£hd_interrupt ÔÚ(kernel/system_call.s,221)¡£
