 ...
 */
 ...
 // 我的理解，原来的软盘相当于现在的U盘一样，容量小但是方便携带。
...
// 软驱的主设备号是 2 ，次设备号 = TYPE*4 + DRIVE ，其中 DRIVE 为 0-3 ，分别对应
// 软驱 A 、 B 、 C 或 D ； TYPE 是软驱的类型， 2 表示 1.2M 软驱， 7 表示 1.44M 软驱
...
extern void floppy_interrupt(void);	// system_call.s 中软驱中断过程标号。
extern char tmp_floppy_area[1024];	// boot/head.s 132 行处定义的软盘缓冲区。
...
unsigned char selected = 0;	// 软驱已选定标志。在处理一个软驱的请求项之前需要首先选定一个软驱。
...
{
repeat:
	floppy_on(nr);	// 启动指定软驱 nr（kernel/sched.c,251）。
	...
	if ((current_DOR & 3) != nr)
		goto repeat;
	// 取数字输入寄存器值，如果最高位（位 7）置位，则表示软盘已更换，
	// 此时关闭马达并退出返回 1。否则关闭马达退出返回 0。
	if (inb(FD_DIR) & 0x80) {
		floppy_off(nr);
		return 1;
	}
...
inline void setup_rw_floppy(void)
{
	...
	output_byte(2);		/* sector size = 512 */	// 参数(字节数(N=2)512 字节)。
...
/* We cannot do a floppy-select, as that might sleep. We just force it */
	...
	// 则需要重新设置 DOR 为当前驱动器current_drive。
	// 定时延迟 2 个滴答时间，然后调用软盘读写传输函数 transfer()。
	// 否则直接调用软盘读写传输函数。
...
void do_fd_request(void)
{
	...
	if (current_drive != CURRENT_DEV)
		seek = 1;
	// 设置读写起始扇区。因为每次读写是以块为单位（1 块为 2 个扇区），所以起始扇区需要起码比
