...
extern int rw_char(int rw,int dev, char * buf, int count, off_t * pos);	//读写在一起
...
int sys_read(unsigned int fd,char * buf,int count)
{
	...
	// 如果文件句柄值大于程序最多打开文件数 NR_OPEN，或者需要读取的字节计数值小于 0，
	...
	// 取文件对应的 i 节点。若是管道文件，并且是读管道文件模式，则进行读管道操作，
	...
	// 如果是目录文件或者是常规文件，则首先验证读取数 count 的有效性并进行调整
	//（若读取字节数加上文件当前读写指针值大于文件大小，
...
// 读文件系统调用函数。
// 参数 fd 是文件句柄，buf 是缓冲区，count 是欲写字节数。
int sys_write(unsigned int fd,char * buf,int count)
{
	struct file * file;
	struct m_inode * inode;
	// 如果文件句柄值大于程序最多打开文件数 NR_OPEN，或者需要写入的字节计数小于 0，
	// 或者该句柄的文件结构指针为空，则返回出错码并退出。
	if (fd>=NR_OPEN || count <0 || !(file=current->filp[fd]))
		return -EINVAL;
	if (!count)	// 若需读取的字节数 count 等于 0，则返回 0，退出
		return 0;
	// 取文件对应的 i 节点。若是管道文件，并且是写管道文件模式，则进行写管道操作，
	// 若成功则返回写入的字节数，否则返回出错码，退出。
	inode=file->f_inode;
	if (inode->i_pipe)
