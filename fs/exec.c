		...
		unsigned long p, int from_kmem)
{
	...
	new_fs = get_ds();	//内核态数据段
	old_fs = get_fs();	//用户态数据段

	// 如果字符串和字符串数组来自内核空间，则设置 fs 段寄存器指向内核数据段（ds）。
	if (from_kmem==2)
		set_fs(new_fs);	//原本fs指向用户的数据段，现在让其指向内核态

	// 循环处理各个参数，从最后一个参数逆向开始复制，复制到指定偏移地址处。
	while (argc-- > 0) {
		if (from_kmem == 1)	// 如果字符串在用户空间而字符串数组在内核空间
			set_fs(new_fs);
		// 从最后一个参数开始逆向操作，取 fs 段中最后一参数指针到 tmp
		...
		while (len) {
			--p; --tmp; --len;
			// 函数刚开始执行时，偏移变量 offset 被初始化为 0，因此若 offset-1<0，
			// 说明是首次复制字符串，则令其等于 p 指针在页面内的偏移值，并申请空闲页面。
			if (--offset < 0) {
				offset = p % PAGE_SIZE;
				if (from_kmem==2)
					set_fs(old_fs);	// 恢复fs
	...
	if (from_kmem==2)
		set_fs(old_fs);	// 恢复fs
	return p;	// 返回参数和环境空间中已复制参数信息的头部偏移值。
}
...
static unsigned long change_ldt(unsigned long text_size,unsigned long * page)
{
	...
	int i;
	// 根据执行文件头部 a_text 值，计算以页面长度为边界的代码段限长。
	// 并设置数据段长度为 64MB。
...
restart_interp:
	...
	// 则后面执行进程的有效用户 ID（euid）就设置为文件的用户 ID，否则设置成当前进程的 euid。
...
/* OK, This is the point of no return */
	...
	// 因此在 322 与 323 行之间需添加一条 if 语句：if (current->sa[I].sa_handler != SIG_IGN)。
	// 这是源代码中的一个 bug。
	for (i=0 ; i<32 ; i++)
	...
	// 修改当前进程各字段为新执行程序的信息。令进程代码段尾值字段 end_code = a_text；
