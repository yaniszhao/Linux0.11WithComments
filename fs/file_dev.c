...
int file_read(struct m_inode * inode, struct file * filp, char * buf, int count)
{
	...
	while (left) {
		...
		// 若 nr 不为 0，则从 i 节点指定的设备上读取该逻辑块，如果读操作失败则退出循环。
		// 若 nr 为 0，表示指定的数据块不存在，置缓冲块指针为 NULL。
		...
		// 并且复制 chars 字节到用户缓冲区 buf 中。否则往用户缓冲区中填入 chars 个 0 值字节。
		if (bh) {
			char * p = nr + bh->b_data;
			while (chars-->0)
				put_fs_byte(*(p++),buf++);
 ...
 */
	...
	while (i<count) {
		...
		// 如果逻辑块号=0，则表示创建失败，退出循环。
		if (!(block = create_block(inode,pos/BLOCK_SIZE)))
			break;
		// 根据该逻辑块号读取设备上的相应数据块，若出错则退出循环。
		if (!(bh=bread(inode->i_dev,block)))
			break;
		// 求出文件读写指针在数据块中的偏移值 c，将 p 指向读出数据块缓冲区中开始读取的位置。
		// 置该缓冲区已修改标志。
		c = pos % BLOCK_SIZE;
		p = c + bh->b_data;
		bh->b_dirt = 1;
		// 从开始读写位置到块末共可写入 c=(BLOCK_SIZE-c)个字节。
		// 若 c 大于剩余还需写入的字节数 (count-i)，则此次只需再写入 c=(count-i)即可。
		c = BLOCK_SIZE-c;
		if (c > count-i) c = count-i;
		// 文件读写指针前移此次需写入的字节数。如果当前文件读写指针位置值超过了文件的大小，
